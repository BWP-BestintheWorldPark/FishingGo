...
#        image_w = 28
#        image_h = 28
#        resize_img = cv2.resize(img, None, fx=image_w /
#                                img.shape[1]*3, fy=image_h/img.shape[0]*3)
...
#        resp = sock.recv(1024)
#        img = cv2.imread(img_path, cv2.IMREAD_COLOR)
#        img = cv2.putText(img, resp.decode(), (50, 50),
...
class Login(QDialog):
    ...
    def login_btn_clicked(self):
        ...
        send_msg = "login/{0}/{1}".format(id, pw)
...
class signup(QDialog):
    ...
    def signup_btn_clicked(self):
        ...
        send_msg = "signup/{0}/{1}/{2}".format(id, pw, name)
        sock.send(send_msg.encode())
        recv_msg = sock.recv(BUF_SIZE)
        recv_msg = recv_msg.decode()
        if "OK" in recv_msg:
            ...
            if(ret) :
                ...
                if self.flag:
                    ...
                    if recv_msg.startswith("send_image"):
                        ...
                        #sock.send(str(len(string_image)).encode()2)
                        #sock.send(string_image) # 서버로 보내기
                        sock.sendall(data)
                        print('바이트 이미지 전송 완료')
                        name = sock.recv(BUF_SIZE).decode()
                        self.fish_name.setText(name)
...
class load(QDialog):
    ...
    def load_btn_clicked(self):
        image = QFileDialog.getOpenFileName(
            self, 'Fish Image Load', './picture', 'Image File(*.png *.jpg *.jpeg)')
        if image[0]:
